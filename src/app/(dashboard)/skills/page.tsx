  ...
  return (
    <div>
      <PageHeader
        ...
        description=""
        onRefresh={load}
        actions={<span className="text-sm text-muted-foreground mr-2">{description}</span>}
      />
          ...
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg text-xs font-medium transition-all duration-300 ${
              filter === f
                ...
                : "glass-card text-muted-foreground hover:text-foreground/60"
            }`}>
      ...
      <div className="flex gap-2 mb-8">
        <div className="relative flex-1 group">
          <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/30 group-focus-within:text-violet-400 transition-colors" />
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            onKeyDown={e => e.key === "Enter" && search()}
            placeholder="Search ClawHub for new skills..."
            className="w-full bg-secondary/50 border border-border rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-violet-500/40 focus:shadow-[0_0_20px_rgba(139,92,246,0.08)] transition-all duration-300 placeholder:text-muted-foreground/30" />
        </div>
      ...
      {searchResults && (
        <div className="glass-card p-5 mb-6 animate-fade-in">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium">ClawHub Results</h3>
            <button onClick={() => setSearchResults("")} className="text-[11px] text-muted-foreground/40 hover:text-muted-foreground/70 transition-colors">dismiss</button>
          </div>
          <pre className="text-xs text-muted-foreground whitespace-pre-wrap font-mono">{searchResults}</pre>
        </div>
      )}

      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="glass-card h-32 animate-shimmer rounded-2xl" />)}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((s, i) => (
            <div key={i} className="glass-card rounded-xl p-5 transition-all duration-300 hover:border-gray-300 dark:hover:border-white/[0.1] hover:-translate-y-0.5 group animate-fade-in-up"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {s.homepage && (
                      <a href={s.homepage} target="_blank" rel="noopener noreferrer"
                        className="text-muted-foreground/20 hover:text-violet-400 transition-colors opacity-0 group-hover:opacity-100">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/50">
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded-md ${
                  s.source === "openclaw-workspace" ? "bg-violet-500/10 text-violet-400/60 border border-violet-500/10" : "bg-secondary text-muted-foreground/40 border border-border/50"
